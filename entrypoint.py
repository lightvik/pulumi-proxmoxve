    ...
    if vm_ips:
        try:
            ...
            if isinstance(ips, dict):
                ...
                console.print(Panel(table, title='[bold]VM IP Адреса[/bold]', border_style='cyan'))
            else:
                console.print(Panel(Text(str(vm_ips)), title='[bold]VM IP Адреса[/bold]', border_style='cyan'))
        except Exception:
            console.print(Panel(Text(vm_ips), title='[bold]VM IP Адреса[/bold]', border_style='cyan'))
...
def main():
    ...
    try:
        ...
        console.print()
        if not Confirm.ask('[bold yellow]Запустить pulumi up?[/bold yellow]', default=False):
