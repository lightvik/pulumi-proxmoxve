...
def build_vm(
    ...
    disks = [
        proxmox.VmLegacyDiskArgs(
            ...
            speed=proxmox.VmLegacyDiskSpeedArgs(
                read=d.speed.read if d.speed else 0,
                read_burstable=d.speed.read_burstable if d.speed else 0,
                write=d.speed.write if d.speed else 0,
                write_burstable=d.speed.write_burstable if d.speed else 0,
                iops_read=d.speed.iops_read if d.speed else 0,
                iops_read_burstable=d.speed.iops_read_burstable if d.speed else 0,
                iops_write=d.speed.iops_write if d.speed else 0,
                iops_write_burstable=d.speed.iops_write_burstable if d.speed else 0,
            ),
            ssd=d.ssd,
        )
    ...
    vm_args["cdrom"] = proxmox.VmLegacyCdromArgs(
        ...
        interface=spec.cdrom.interface if spec.cdrom else "ide0",
